...
class VisionPage(QFrame):
    ...
    # ── 统一卡片工厂：页面只负责内容，外观全部由 theme.qss 控制 ──
    @staticmethod
    ...
    def _build_ui(self):
        ...
        eyebrow = QLabel("VISION / LIVE")
        eyebrow.setObjectName("PageEyebrow")
        title = QLabel("视觉识别工作台")
        title.setObjectName("PageTitle")
        subtitle = QLabel("摄像头手势识别 · O6 六维姿态映射")
        ...
        self.cam_view.setObjectName("VisionCameraView")
        ...
        for button in (self.btn_start, self.btn_stop, self.btn_home):
            button.setCursor(Qt.PointingHandCursor)
            button.setMinimumHeight(34)
        primary_layout.addWidget(self.btn_start)
        primary_layout.addWidget(self.btn_stop)
        primary_layout.addWidget(self.btn_home)
        primary_layout.addStretch()
        mode_note = QLabel("识别默认仅预览")
        mode_note.setObjectName("ActionBarNote")
        primary_layout.addWidget(mode_note)
        left_layout.addWidget(primary_bar)
        ...
        # 1. 核心模仿控制
        mimic_card, mimic = self._section_card("模仿控制", "先完成张开/握拳校准，再按需启用机械手下发。")
        output_row = QFrame()
        ...
        output_title = QLabel("机械手实时同步")
        output_title.setObjectName("ToggleTitle")
        output_note = QLabel("关闭时只进行视觉识别，不发送动作")
        ...
        for button in (self.btn_cal_open, self.btn_cal_close, self.btn_test_open, self.btn_test_close):
            button.setProperty("category", "secondary")
            button.setProperty("size", "compact")
            button.setCursor(Qt.PointingHandCursor)
            button.setMinimumHeight(31)
        self.btn_cal_open.setProperty("accent", True)
        self.btn_cal_close.setProperty("accent", True)
        calibration_grid.addWidget(self.btn_cal_open, 0, 0)
        calibration_grid.addWidget(self.btn_cal_close, 0, 1)
        calibration_grid.addWidget(self.btn_test_open, 1, 0)
        calibration_grid.addWidget(self.btn_test_close, 1, 1)
        mimic.addLayout(calibration_grid)
        right.addWidget(mimic_card)

        # 2. 手势映射
        mapping_card, mapping = self._section_card("手势映射", "分组校准拇指方向，并对六个控制通道进行单步验证。")
        thumb_group = QFrame()
        thumb_group.setObjectName("InlineGroup")
        thumb_layout = QVBoxLayout(thumb_group)
        thumb_layout.setContentsMargins(10, 8, 10, 9)
        thumb_layout.setSpacing(7)
        thumb_label = QLabel("拇指方向校准")
        thumb_label.setObjectName("SubsectionLabel")
        thumb_layout.addWidget(thumb_label)
        thumb_buttons = QHBoxLayout()
        thumb_buttons.setSpacing(7)
        self.btn_cal_thumb_open = QPushButton("拇指外展")
        self.btn_cal_thumb_open.setToolTip("Cal thumb out")
        self.btn_cal_thumb_close = QPushButton("拇指内收")
        self.btn_cal_thumb_close.setToolTip("Cal thumb in")
        for button in (self.btn_cal_thumb_open, self.btn_cal_thumb_close):
            button.setProperty("category", "secondary")
            button.setProperty("size", "compact")
            button.setCursor(Qt.PointingHandCursor)
            thumb_buttons.addWidget(button)
        thumb_layout.addLayout(thumb_buttons)
        mapping.addWidget(thumb_group)

        finger_group = QFrame()
        finger_group.setObjectName("InlineGroup")
        finger_layout = QVBoxLayout(finger_group)
        finger_layout.setContentsMargins(10, 8, 10, 9)
        finger_layout.setSpacing(7)
        finger_label = QLabel("控制通道测试")
        finger_label.setObjectName("SubsectionLabel")
        ...
        finger_buttons = [
            ...
            ("无名", "ring", [OPEN_POSE[0], OPEN_POSE[1], OPEN_POSE[2], OPEN_POSE[3], CLOSE_POSE[4], OPEN_POSE[5]]),
            ("小指", "little", [OPEN_POSE[0], OPEN_POSE[1], OPEN_POSE[2], OPEN_POSE[3], OPEN_POSE[4], CLOSE_POSE[5]]),
        ]
        self._finger_test_btns = {}
        for index, (label, key, pose) in enumerate(finger_buttons):
            button = QPushButton(label)
            button.setProperty("category", "tool")
            button.setProperty("variant", "channel")
            button.setCursor(Qt.PointingHandCursor)
            button.clicked.connect(lambda checked=False, name=key, values=pose: self._send_o6_test_pose(name, values))
            finger_grid.addWidget(button, index // 3, index % 3)
            self._finger_test_btns[key] = button
        finger_layout.addLayout(finger_grid)
        mapping.addWidget(finger_group)

        invert_row = QFrame()
        invert_row.setObjectName("InlineGroup")
        invert_layout = QHBoxLayout(invert_row)
        invert_layout.setContentsMargins(10, 7, 10, 7)
        invert_title = QLabel("方向反转")
        invert_title.setObjectName("SubsectionLabel")
        invert_layout.addWidget(invert_title)
        invert_layout.addStretch()
        self.chk_thumb_bend_invert = QCheckBox("弯曲反向")
        self.chk_thumb_bend_invert.setChecked(self._thumb_bend_invert)
        self.chk_thumb_invert = QCheckBox("横摆反向")
        self.chk_thumb_invert.setChecked(self._thumb_swing_invert)
        invert_layout.addWidget(self.chk_thumb_bend_invert)
        invert_layout.addWidget(self.chk_thumb_invert)
        mapping.addWidget(invert_row)
        right.addWidget(mapping_card)

        # 3. 高级参数
        tuning_card, tuning = self._section_card("高级参数", "平滑、死区、发送间隔与最大单步限制。")
        ...
        self.spin_db = QSpinBox(); self.spin_db.setRange(0, 30); self.spin_db.setValue(self._deadband)
        self.spin_iv = QDoubleSpinBox(); self.spin_iv.setRange(0.03, 0.50); self.spin_iv.setSingleStep(0.01); self.spin_iv.setDecimals(2); self.spin_iv.setValue(self._emit_iv)
        self.spin_fs = QSpinBox(); self.spin_fs.setRange(1, 80); self.spin_fs.setValue(MAX_STEP_FINGER)
        self.spin_ts = QSpinBox(); self.spin_ts.setRange(1, 80); self.spin_ts.setValue(MAX_STEP_THUMB_SWING)
        param_grid = QGridLayout()
        param_grid.setSpacing(7)
        params = [
            ("EMA", "平滑系数", self.spin_ema),
            ("DB", "变化死区", self.spin_db),
            ("IV", "发送间隔", self.spin_iv),
            ("Fstep", "手指步长", self.spin_fs),
            ("Tstep", "拇指步长", self.spin_ts),
        ]
        for index, (code, label, spin) in enumerate(params):
            cell = QFrame()
            cell.setObjectName("ParamCell")
            cell_layout = QVBoxLayout(cell)
            cell_layout.setContentsMargins(9, 7, 9, 8)
            cell_layout.setSpacing(4)
            name = QLabel(f"{label}  ·  {code}")
            name.setObjectName("ParamCellLabel")
            spin.setObjectName("ParamCellInput")
            spin.setMinimumHeight(28)
            cell_layout.addWidget(name)
            cell_layout.addWidget(spin)
            row, column = divmod(index, 2)
            param_grid.addWidget(cell, row, column)
        param_grid.setColumnStretch(0, 1)
        param_grid.setColumnStretch(1, 1)
        tuning.addLayout(param_grid)
        right.addWidget(tuning_card)

        # 4. 识别摘要与日志
        status_card, status = self._section_card("识别状态", "关键状态常驻展示，详细流水收纳在下方日志。")
        summary_grid = QGridLayout()
        summary_grid.setSpacing(7)
        summary_defs = [("当前手势", "hand"), ("机械手下发", "hw"), ("识别频率", "freq")]
        self.d_hand = QLabel("等待")
        self.d_hw = QLabel("未启用")
        self.d_freq = QLabel("--")
        for index, (label, key) in enumerate(summary_defs):
            tile = QFrame()
            tile.setObjectName("StatusMetric")
            tile.setProperty("kind", key)
            tile_layout = QVBoxLayout(tile)
            tile_layout.setContentsMargins(8, 7, 8, 7)
            tile_layout.setSpacing(2)
            caption = QLabel(label)
            caption.setObjectName("StatusMetricLabel")
            value = {"hand": self.d_hand, "hw": self.d_hw, "freq": self.d_freq}[key]
            value.setObjectName("StatusMetricValue")
            tile_layout.addWidget(caption)
            tile_layout.addWidget(value)
            summary_grid.addWidget(tile, 0, index)
        status.addLayout(summary_grid)
        self.log_view = QTextEdit()
        self.log_view.setObjectName("VisionLogView")
        self.log_view.setReadOnly(True)
        self.log_view.setMaximumHeight(112)
        self.log_view.setPlaceholderText("识别日志将在这里显示")
        self._log_lines = []
        status.addWidget(self.log_view)
        right.addWidget(status_card)

        # 5. 录制与回放
        record_card, record = self._section_card("录制与回放", "录制、文件管理和回放控制按任务分组。")
        record_group = QFrame()
        record_group.setObjectName("InlineGroup")
        record_group_layout = QVBoxLayout(record_group)
        record_group_layout.setContentsMargins(10, 8, 10, 9)
        record_group_layout.setSpacing(7)
        record_label = QLabel("动作录制")
        record_label.setObjectName("SubsectionLabel")
        record_group_layout.addWidget(record_label)
        record_buttons = QGridLayout()
        record_buttons.setSpacing(6)
        ...
        self.btn_record_clear = QPushButton("清空录制")
        self.btn_record_clear.setProperty("category", "tool")
        for index, button in enumerate((self.btn_record_start, self.btn_record_stop, self.btn_record_clear)):
            button.setProperty("size", "compact")
            button.setCursor(Qt.PointingHandCursor)
            record_buttons.addWidget(button, 0, index)
        record_group_layout.addLayout(record_buttons)
        record.addWidget(record_group)

        file_group = QFrame()
        file_group.setObjectName("InlineGroup")
        file_layout = QHBoxLayout(file_group)
        file_layout.setContentsMargins(10, 8, 10, 8)
        file_label = QLabel("录制文件")
        file_label.setObjectName("SubsectionLabel")
        file_layout.addWidget(file_label)
        file_layout.addStretch()
        self.btn_record_save = QPushButton("保存")
        self.btn_record_load = QPushButton("加载")
        for button in (self.btn_record_save, self.btn_record_load):
            button.setProperty("category", "secondary")
            button.setProperty("size", "compact")
            button.setCursor(Qt.PointingHandCursor)
            file_layout.addWidget(button)
        record.addWidget(file_group)

        playback_group = QFrame()
        playback_group.setObjectName("InlineGroup")
        playback_layout = QVBoxLayout(playback_group)
        playback_layout.setContentsMargins(10, 8, 10, 9)
        playback_layout.setSpacing(7)
        playback_label = QLabel("动作回放")
        playback_label.setObjectName("SubsectionLabel")
        playback_layout.addWidget(playback_label)
        playback_buttons = QGridLayout()
        playback_buttons.setSpacing(6)
        ...
        self.btn_play_stop = QPushButton("停止")
        self.btn_play_stop.setProperty("category", "danger")
        for index, button in enumerate((self.btn_play_start, self.btn_play_pause, self.btn_play_stop)):
            button.setProperty("size", "compact")
            button.setCursor(Qt.PointingHandCursor)
            playback_buttons.addWidget(button, 0, index)
        playback_layout.addLayout(playback_buttons)
        playback_options = QHBoxLayout()
        playback_options.setSpacing(8)
        ...
        self.cmb_play_speed.setMinimumWidth(76)
        playback_options.addWidget(self.chk_play_loop)
        playback_options.addStretch()
        speed_label = QLabel("速度")
        speed_label.setObjectName("FieldLabel")
        playback_options.addWidget(speed_label)
        playback_options.addWidget(self.cmb_play_speed)
        playback_layout.addLayout(playback_options)
        record.addWidget(playback_group)

        record_status = QFrame()
        record_status.setObjectName("RecordStatusBar")
        record_status_layout = QHBoxLayout(record_status)
        record_status_layout.setContentsMargins(9, 6, 9, 6)
        record_status_layout.setSpacing(12)
        self.d_rec_state = QLabel("录制 · 未开始")
        self.d_play_state = QLabel("回放 · 未开始")
        self.d_rec_state.setObjectName("RecordState")
        self.d_play_state.setObjectName("PlaybackState")
        record_status_layout.addWidget(self.d_rec_state)
        record_status_layout.addWidget(self.d_play_state)
        record_status_layout.addStretch()
        record.addWidget(record_status)
        right.addWidget(record_card)
        right.addStretch()
        main.addWidget(right_scroll, stretch=4)

        # 保留业务逻辑引用的状态对象；仅改变呈现，不改变读写关系。
