...
- 关节曲线使用图例点击模式：显示所有关节名，点击切换显示/隐藏，至少保留一个可见关节；不同关节使用 `CURVE_COLORS` 区分颜色。
- 侧边栏红点：`Diagnostics` 通过可选的 `onAlertChange` 回调与应用壳通信——挂载时清除红点；存在 error/warn 日志、连接异常或遥测断线时，卸载后重新亮起红点。红点不是常驻标识，只反映需要关注的状态。
