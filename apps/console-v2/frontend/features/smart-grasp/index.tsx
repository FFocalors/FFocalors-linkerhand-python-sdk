...
export function SmartGrasp({ grasp, locked, tactileAvailable: _tactileAvailable, model = 'O6', controller }: { grasp: GraspPort; locked: boolean; tactileAvailable: boolean; model?: Model; controller?: GraspController }) {
  ...
  return <div className="stack">
    ...
    <Card><div className="card-header"><div><h2>操作流程</h2><span className="muted">当前阶段：{phaseLabel}</span></div><div className="heading-actions"><Badge tone={state.phase === 'failed' || state.phase === 'aborted' ? 'red' : state.phase === 'holding' ? 'green' : 'blue'}>{phaseLabel}</Badge>{controllerReady && state.phase !== 'idle' && state.phase !== 'ready' && state.phase !== 'aborted' && state.phase !== 'failed' && <button className="button button-ghost" onClick={() => void invoke(() => controller!.abort(), '中止请求失败，请立即使用顶部停止全部动作。')}>中止</button>}</div></div><Progress value={state.phase === 'idle' ? 0 : state.phase === 'calibrating' ? 18 : state.phase === 'ready' ? 35 : state.phase === 'approach' ? 55 : state.phase === 'grasping' ? 72 : state.phase === 'holding' ? 88 : state.phase === 'releasing' ? 65 : 0} /><div className="heading-actions"><button className="button button-secondary" disabled={locked || !controllerReady || !available || state.phase !== 'idle'} onClick={() => void invoke(() => controller!.calibrate(), '标定启动失败。')}>1. 开始标定</button><button className="button button-secondary" disabled={locked || !controllerReady || state.phase !== 'calibrating'} onClick={() => void invoke(() => controller!.completeCalibration(), '标定完成失败。')}>2. 标定完成</button><button className="button button-secondary" disabled={locked || !controllerReady || state.phase !== 'ready'} onClick={() => void invoke(() => controller!.approach(), '接近阶段启动失败。')}>3. 接近目标</button><button className="button button-secondary" disabled={!canRun || state.phase !== 'approach'} onClick={run}>4. 抓取</button><button className="button button-ghost" disabled={!controllerReady || state.phase !== 'holding'} onClick={() => void invoke(() => controller!.release(), '释放请求失败。')}>释放</button></div></Card>
