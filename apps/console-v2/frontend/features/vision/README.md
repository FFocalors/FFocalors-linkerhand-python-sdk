...
- 骨架显示直接使用每一帧有效的 21 点结果，不再被“张开/握拳/未知”的离散手势分类门控；半握拳属于正常连续姿态。
- 单帧检测缺失只对骨架保留 150ms 视觉宽限，避免闪断；真实设备建议会立即撤销，不会把旧姿态继续下发。
- 画布按视频 `object-fit: contain` 的实际内容矩形投影关键点，4:3 摄像头位于宽屏卡片时不再发生骨架与画面错位。
- 右侧状态卡最多约 10Hz 更新，骨架仍由动画帧读取最新结果，避免整页频繁 React 重渲染阻塞摄像头帧捕获。
