...
describe('RPS deterministic controller', () => {
  ...
  it('reports busy runtime and does not stop a runtime owned by vision', async () => { const runtime = new FakeRuntime(); runtime.emitSnapshot({ state: 'running', owner: 'vision', model: 'ready' }); const controller = new RpsGameController({ runtime, capabilities: capabilities('L7') }); controller.attach(fakeVideo); expect(controller.snapshot().phase).toBe('idle'); expect(controller.snapshot().cameraError?.code).toBe('VISION_BUSY'); await controller.stop(); expect(runtime.stops).toBe(0); });
  ...
  it('keeps late authorization and dispatch promises from reviving a cancelled session', async () => {
    ...
    const firstAuth = controller.authorizeHardware(); controller.lock(); resolveAuth(true); await firstAuth; expect(controller.snapshot().hardwareAuthorized).toBe(false);
