...
export const JointSlider = memo(function JointSlider({ index, value, disabled, onBegin, onInput, onFinish }: JointSliderProps) {
  ...
  return <label className="joint-row"><span className="joint-name">J{index + 1}</span><input ref={inputRef} aria-label={`J${index + 1} 目标`} type="range" min="0" max="1" step="0.01" defaultValue={value} disabled={disabled} onPointerDown={() => { draggingRef.current = true; onBegin(index); }} onPointerUp={() => { draggingRef.current = false; onFinish(index); }} onPointerCancel={() => { draggingRef.current = false; onFinish(index); }} onBlur={() => { if (draggingRef.current) { draggingRef.current = false; onFinish(index); } }} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); draggingRef.current = false; onFinish(index, true); } }} onChange={handleChange} /><output ref={outputRef}>{Math.round(value * 100)}%</output></label>;
});
...
export function DeviceControl({ device, telemetry, config, capabilities, locked = false, controller, quickActions = [{ id: 'safe-position', label: '回到安全位', detail: '由设备控制器执行' }], loops = [], onNavigateToDiagnostics }: DeviceControlProps) {
  ...
  const singleQuickOperation = quickOperationActive && quickActions.length === 1;
  ...
  return <div className="stack">
    ...
    <Card><div className="card-header"><div><h2>关节目标</h2><span className="muted">拖动中保留本地目标；松开、失焦或 Enter / Space 提交最终完整向量。</span></div><span className="muted">归一化命令域 · 0–100%</span></div>{groups.length === 0 && <p className="muted">当前能力未报告关节，控制不可用。</p>}<div className="joint-groups">{groups.map((group, groupIndex) => <details key={group.start} open={jointCount <= 10 || groupIndex === 0}><summary>关节 {group.start + 1}–{group.end}<span className="muted">{group.end - group.start} 个</span></summary><div className="joint-list">{Array.from({ length: group.end - group.start }, (_, offset) => group.start + offset).map(index => <JointSlider key={index} index={index} value={values[index] ?? 0} disabled={!controller || isLocked || connection.state !== 'connected'} onBegin={beginJoint} onInput={changeJoint} onFinish={finishJoint} />)}</div></details>)}</div><details open={rawOpen} onToggle={event => setRawOpen(event.currentTarget.open)}><summary>高级诊断：原始值估算 <span className="muted">仅命令域换算，不代表设备当前原始反馈</span></summary><div className="grid grid-3" style={{ marginTop: 12 }}><span className="muted">position raw 估算：{values.map(value => Math.round(value * (capabilities.position.range.max - capabilities.position.range.min) + capabilities.position.range.min)).join(', ') || '—'}</span><span className="muted">范围：{capabilities.position.range.min}–{capabilities.position.range.max}</span><span className="muted">遥测 raw position：{live?.rawPosition.join(', ') || '—'}</span></div></details></Card>
    ...
    <div className="grid grid-2"><Card><div className="card-header"><div><h2>快速动作</h2><span className="muted">真实执行状态由 controller 返回</span></div></div><div className="grid grid-2" style={{ marginTop: 16 }}>{quickActions.map(action => <button className="button button-secondary" key={action.id} disabled={!controller || isLocked || busy || Boolean(submittingQuickAction || submittingLoop || quickOperationActive || loopOperationActive)} onClick={() => { setSubmittingQuickAction(action.id); void runController(() => controller!.startQuickAction(action.id)).finally(() => setSubmittingQuickAction(undefined)); }}>{submittingQuickAction === action.id || singleQuickOperation || (quickOperationActive && operationKind.includes(action.id.toLowerCase())) ? `${action.label} · 执行中` : action.label}</button>)}<button className="button button-ghost" disabled={!controller || !quickOperationActive} onClick={() => { void runController(controller!.stopQuickAction); }}>停止快速动作</button></div></Card><Card><div className="card-header"><div><h2>动作循环</h2><span className="muted">循环不会在浏览器定时发送硬件命令</span></div></div>{loops.length === 0 ? <p className="muted" style={{ marginTop: 18 }}>控制器尚未提供循环方案。</p> : <div className="grid grid-2" style={{ marginTop: 16 }}>{loops.map(loop => <button className="button button-secondary" key={loop.id} disabled={!controller || isLocked || busy || Boolean(submittingLoop || submittingQuickAction || quickOperationActive || loopOperationActive)} onClick={() => { setSubmittingLoop(loop.id); void runController(() => controller!.startLoop(loop.id)).finally(() => setSubmittingLoop(undefined)); }}>{submittingLoop === loop.id || singleLoopOperation || (loopOperationActive && operationKind.includes(loop.id.toLowerCase())) ? `${loop.label} · 循环中` : loop.label}</button>)}<button className="button button-ghost" disabled={!controller || !loopOperationActive} onClick={() => { void runController(controller!.stopLoop); }}>停止循环</button></div>}</Card></div><p className="muted">停止全部动作是软件锁定，不是物理断电急停；危险场景请使用设备的物理急停装置。</p>
  </div>;
}
