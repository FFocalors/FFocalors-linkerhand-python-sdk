...
- 页面右上角“恢复初始状态”执行共享的 O6“张开”预设，并同步关节目标显示；软件安全锁仍由应用最上方的全局停止按钮统一管理。
...
- 调试模式 ON + 未连接物理手：应用壳注入的虚拟手强制连接为 `connected`，设备读数、曲线和动作中心草稿共享同一个持续虚拟遥测流；连接/断开/重连、关节、速度/扭矩、快捷动作和恢复初始状态均作用于虚拟手（跳过真实设备调用），并显示「调试模式：操作作用于虚拟调试机械手」提示。
