...
export function Settings({ model, transport, controller, themePort, version = '0.1.0', build = 'dev' }: SettingsProps) {
  ...
  const applySnapshot = useCallback((value: SettingsSnapshot | DeviceConfig) => { const next = normaliseSnapshot(value); setSnapshot(previous => ({ ...previous, ...next, config: next.config })); setDraft(previous => savedDraftRef.current ? previous : draftFromSnapshot(next)); if (next.cameras) setCameras(next.cameras); if (next.cameraPermission) setPermission(next.cameraPermission); if (next.theme) setTheme(next.theme); if (next.version) setSnapshotVersion(next.version); }, []);

  useEffect(() => {
    let active = true;
    if (!controller) { setStatus('saved'); return () => { active = false; }; }
    void controller.load().then(value => { if (!active) return; const next = normaliseSnapshot(value); setSnapshot(next); const nextDraft = draftFromSnapshot(next); setDraft(nextDraft); savedDraftRef.current = nextDraft; setSavedDraft(nextDraft); setPermission(next.cameraPermission ?? 'unknown'); setCameras(next.cameras ?? []); setSnapshotVersion(next.version ?? version); setStatus('saved'); }).catch(error => { if (active) { setStatus('error'); setMessage(`读取设置失败：${errorText(error)}`); } });
    const unsubscribe = controller.subscribe(value => { if (active) applySnapshot(value); });
    return () => { active = false; unsubscribe(); };
  }, [applySnapshot, controller, version]);
  useEffect(() => { if (!themePort) return undefined; let active = true; void Promise.resolve(themePort.getTheme()).then(value => { if (active) setTheme(value); }).catch(() => undefined); const unsubscribe = themePort.subscribe?.(value => setTheme(value)); return () => { active = false; unsubscribe?.(); }; }, [themePort]);

  const setDraftValue = <K extends keyof SettingsDraft>(key: K, value: SettingsDraft[K]) => { setDraft(previous => ({ ...previous, [key]: value })); setStatus('dirty'); };
  ...
  const validate = async () => { const result = await Promise.resolve(controller?.validate(draft) ?? validateSettingsDraft(draft)); setErrors(result.errors); return result.valid; };
  const save = async () => { if (!controller || !(await validate())) return; setStatus('saving'); setMessage(''); try { const result = await controller.save(draft); setErrors(result.errors.reduce<Record<string, string>>((all, item) => ({ ...all, save: item }), {})); if (result.errors.length) { setStatus('error'); setMessage(result.errors.join('；')); return; } savedDraftRef.current = draft; setSavedDraft(draft); setStatus(result.restartRequired ? 'restart' : result.reconnectRequired ? 'reconnect' : result.applied ? 'applied' : 'saved'); setMessage(result.restartRequired ? '已保存。需要重启服务后生效。' : result.reconnectRequired ? '已保存。需要重新连接设备后生效。' : result.applied ? '已保存并由运行时确认应用。' : '已保存，等待运行时确认。'); } catch (error) { setStatus('error'); setMessage(`保存失败：${errorText(error)}`); } };
  const runCheck = async (kind: 'sidecar' | 'offline' | 'camera') => { if (!controller) return; setBusyCheck(kind); setMessage(''); try { if (kind === 'camera') { const result = await controller.listCameras(); setCameras(result.cameras); setPermission(result.permission); setMessage(result.permission === 'denied' ? '摄像头权限被拒绝。' : `已发现 ${result.cameras.length} 个摄像头。`); } else { const result = kind === 'sidecar' ? await controller.testSidecar() : await controller.checkOfflineAssets(); setMessage(`${result.ok ? '检查通过' : '检查未通过'}：${result.message}${result.detail ? `（${result.detail}）` : ''}`); } } catch (error) { setMessage(`检查失败：${errorText(error)}`); if (kind === 'camera') setPermission('error'); } finally { setBusyCheck(undefined); } };
  const dirty = status === 'dirty' || Boolean(savedDraft && JSON.stringify(savedDraft) !== JSON.stringify(draft));
  const statusLabel = status === 'loading' ? '读取中' : status === 'saving' ? '保存中' : status === 'dirty' ? '未保存' : status === 'reconnect' ? '需要重连' : status === 'restart' ? '需要重启服务' : status === 'error' ? '检查失败' : status === 'applied' ? '已应用' : '已保存';
  ...
  return <div className="stack settings-feature">
    <div className="page-heading"><div><p className="eyebrow">系统 / 设置</p><h1>设置</h1><p>保存的是设备配置草稿；硬件状态仍以运行时返回为准。</p></div><div className="settings-heading-status"><Badge tone={!wired ? 'amber' : status === 'error' ? 'red' : status === 'dirty' ? 'amber' : 'green'}>{!wired ? '未接线' : statusLabel}</Badge><button className="button button-primary" disabled={!wired || status === 'saving' || status === 'loading'} onClick={() => void save()}>保存设置</button></div></div>
    ...
    <div className="settings-grid">
      <Card><div className="card-header"><div><h2>设备连接</h2><span className="muted">所有字段进入 staged draft，提交前不会改变运行中设备。</span></div><Badge>{transportLabel(draft.transport)}</Badge></div><div className="settings-fields">
        <label>设备型号<select value={draft.model} disabled={!wired} onChange={event => setDraftValue('model', event.target.value as SettingsModel)}>{DEVICE_MODELS.map(item => <option key={item} value={item}>{item}</option>)}</select></label>
        <fieldset><legend>左右手</legend><div className="settings-options"><label><input type="radio" name="hand" checked={draft.hand === 'left'} disabled={!wired} onChange={() => setDraftValue('hand', 'left')} />左手</label><label><input type="radio" name="hand" checked={draft.hand === 'right'} disabled={!wired} onChange={() => setDraftValue('hand', 'right')} />右手</label></div></fieldset>
        <fieldset><legend>传输方式</legend><div className="transport-switch"><button type="button" className={draft.transport.type === 'can' ? 'selected' : ''} disabled={!wired} onClick={() => { setDraft(switchTransport(draft, 'can')); setStatus('dirty'); }}>CAN</button><button type="button" className={draft.transport.type === 'rs485' ? 'selected' : ''} disabled={!wired} onClick={() => { setDraft(switchTransport(draft, 'rs485')); setStatus('dirty'); }}>RS485</button></div></fieldset>
        {draft.transport.type === 'can' ? <label>CAN channel<input value={draft.transport.channel} disabled={!wired} onChange={event => setTransport({ type: 'can', channel: event.target.value })} aria-invalid={Boolean(errors['transport.channel'])} />{errors['transport.channel'] && <small className="field-error">{errors['transport.channel']}</small>}</label> : rs485Transport ? <div className="settings-two-fields"><label>串口<input value={rs485Transport.port} disabled={!wired} onChange={event => setTransport({ type: 'rs485', port: event.target.value, baudrate: rs485Transport.baudrate })} aria-invalid={Boolean(errors['transport.port'])} />{errors['transport.port'] && <small className="field-error">{errors['transport.port']}</small>}</label><label>波特率<select value={rs485Transport.baudrate} disabled={!wired} onChange={event => setTransport({ type: 'rs485', port: rs485Transport.port, baudrate: Number(event.target.value) })}>{[9600, 19200, 38400, 57600, 115200, 230400, 460800, 921600].map(rate => <option key={rate} value={rate}>{rate}</option>)}</select>{errors['transport.baudrate'] && <small className="field-error">{errors['transport.baudrate']}</small>}</label></div> : null}
        <p className="muted settings-hint">{transportHint}</p>
      </div></Card>
      <Card><div className="card-header"><div><h2>摄像头</h2><span className="muted">只保存首选 deviceId；不会创建 VisionRuntime 或打开第二个摄像头。</span></div><Badge tone={permission === 'granted' ? 'green' : permission === 'denied' ? 'red' : 'amber'}>{permission === 'granted' ? '权限已允许' : permission === 'denied' ? '权限被拒绝' : permission === 'error' ? '枚举失败' : '权限未确认'}</Badge></div><div className="camera-controls"><label>首选摄像头<select value={cameraSelection} disabled={!wired || cameras.length === 0} onChange={event => setDraftValue('preferredCameraDeviceId', event.target.value || null)}><option value="">不指定</option>{cameras.map(camera => <option value={camera.deviceId} key={camera.deviceId}>{camera.label || camera.deviceId}</option>)}</select></label><button className="button button-secondary" disabled={!wired || busyCheck === 'camera'} onClick={() => void runCheck('camera')}>{busyCheck === 'camera' ? '枚举中…' : '刷新摄像头'}</button></div>{cameras.length === 0 && <p className="muted camera-empty">尚未枚举摄像头。请点击刷新；权限和错误均来自 controller。</p>}</Card>
      <Card><div className="card-header"><div><h2>外观</h2><span className="muted">主题由应用 ThemePort 持有。</span></div><Badge>{theme === 'system' ? '跟随系统' : theme === 'light' ? '浅色' : '深色'}</Badge></div><div className="theme-options" role="radiogroup" aria-label="主题"><label className={theme === 'light' ? 'selected' : ''}><input type="radio" name="theme" checked={theme === 'light'} onChange={() => setThemePreference('light')} />浅色</label><label className={theme === 'dark' ? 'selected' : ''}><input type="radio" name="theme" checked={theme === 'dark'} onChange={() => setThemePreference('dark')} />深色</label><label className={theme === 'system' ? 'selected' : ''}><input type="radio" name="theme" checked={theme === 'system'} onChange={() => setThemePreference('system')} />跟随系统</label></div></Card>
