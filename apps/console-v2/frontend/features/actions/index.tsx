...
export function ActionCenter({ actions, motion: _motion, locked, controller }: { actions: ActionPort; motion: MotionPort; locked: boolean; controller?: ActionController }) {
  ...
  return <div className="stack">
    <div className="page-heading"><div><p className="eyebrow">编排 / 动作中心</p><h1>动作中心</h1><p>把经过验证的操作保存为可复用动作，所有执行状态来自 ActionController。</p></div><button className="button button-primary" disabled={locked || !controllerReady || recording || drafting} onClick={() => setDrafting(true)}>＋ 新建动作</button></div>
