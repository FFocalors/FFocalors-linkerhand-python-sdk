...
function fakeWorker(init: 'ready' | 'error' = 'ready') {
  ...
  worker = { onmessage: null, onerror: null, terminate: vi.fn(), frames: [], postMessage: vi.fn((message: { type: string; requestId: string; bitmap?: ImageBitmap }) => {
    if (message.type === 'init') queueMicrotask(() => worker.onmessage?.({ data: init === 'ready' ? { type: 'ready', requestId: message.requestId } : { type: 'error', requestId: message.requestId, code: 'MODEL_LOAD_FAILED', message: 'fixture model error' } } as MessageEvent));
    if (message.type === 'frame') worker.frames.push({ requestId: message.requestId, bitmap: message.bitmap! });
  }) };
  return { worker, emit: (data: unknown) => { const message = data as { type?: string; requestId?: string }; if (message.type === 'error') worker.frames.find(frame => frame.requestId === message.requestId)?.bitmap.close(); worker.onmessage?.({ data } as MessageEvent); }, fail: (message = 'fixture worker error') => worker.onerror?.({ message } as ErrorEvent) };
}
