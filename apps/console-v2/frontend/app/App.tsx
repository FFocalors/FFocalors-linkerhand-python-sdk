...
function Shell() {
  ...
  const runtime: ConsolePorts = isTauriRuntime() ? tauriRuntime : mockRuntime;
  useEffect(() => { void Promise.all([runtime.device.getConfig(), runtime.device.getCapabilities()]).then(([nextConfig, nextCapabilities]) => { setConfig(nextConfig); setCapabilities(nextCapabilities); }); }, [runtime]);
  ...
  const renderPage = () => { switch (page) { case 'device': return <DeviceControl device={runtime.device} telemetry={runtime.telemetry} config={config} capabilities={capabilities} locked={locked} />; case 'grasp': return <SmartGrasp grasp={runtime.grasp} tactileAvailable={capabilities.touch.available} locked={locked} />; case 'vision': return <VisionMimic vision={runtime.vision} capabilities={capabilities} locked={locked} />; case 'rps': return <RockPaperScissors vision={runtime.vision} capabilities={capabilities} locked={locked} />; case 'actions': return <ActionCenter actions={runtime.actions} motion={runtime.motion} locked={locked} />; case 'diagnostics': return <Diagnostics logs={runtime.logs} />; case 'settings': return <Settings model={config.model} transport={config.transport} />; } };
  const stopAll = async () => { await runtime.device.stopAll(); setLocked(true); };
  const unlock = async () => { await runtime.device.unlock(); setLocked(false); };
