...
function Shell() {
  ...
  const renderPage = () => { switch (page) { case 'device': return <DeviceControl device={mockRuntime.device} telemetry={mockRuntime.telemetry} config={config} capabilities={capabilities} locked={locked} />; case 'grasp': return <SmartGrasp grasp={mockRuntime.grasp} tactileAvailable={capabilities.touch.available} locked={locked} />; case 'vision': return <VisionMimic vision={mockRuntime.vision} capabilities={capabilities} locked={locked} />; case 'rps': return <RockPaperScissors vision={mockRuntime.vision} capabilities={capabilities} locked={locked} />; case 'actions': return <ActionCenter actions={mockRuntime.actions} motion={mockRuntime.motion} locked={locked} />; case 'diagnostics': return <Diagnostics logs={mockRuntime.logs} />; case 'settings': return <Settings model={config.model} transport={config.transport} />; } };
