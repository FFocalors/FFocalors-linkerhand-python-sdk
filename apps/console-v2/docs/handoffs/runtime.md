...
- Commit: `c1532ece4ea0179531fe87634a8ee0d599ab310e`
...
- Public interfaces: added versioned camelCase DTOs and `WireEnvelope`; adapter, motion, telemetry, sidecar, and typed application ports are new. No existing public interface was changed because this was an empty baseline.
- Verification: `cargo fmt --all`; `cargo test --workspace` (all unit/doc tests pass); `cargo clippy --workspace --all-targets -- -D warnings` passes on the current Windows MSVC target.
